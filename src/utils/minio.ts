...
export class MinioUtils {
   ...
   */
  static async fetchImageAndUpload(
    src: string,
    bucketName: string,
    uploadDir?: string
  ) {
  ...
  static async gacha(bucketName: string, dir?: string): Promise<string> {
    ...
    const files: Minio.BucketItem[] = await new Promise((resolve, reject) => {
      ...
      const stream: Minio.BucketStream<Minio.BucketItem> =
        instance.listObjectsV2(bucketName, dir, true)
      stream.on('data', obj => objects.push(obj))
      stream.on('end', () => resolve(objects))
      stream.on('error', err => reject(err))
    })

    const randomFile: string =
      files[Math.floor(Math.random() * files.length)].name
    logger.info(`抽卡 - 抽取到: ${randomFile}`)
    return `${this.config.useSSL ? 'https' : 'http'}://${
      this.config.endPoint
    }:${this.config.port}/${bucketName}/${randomFile}`
  }

  static async gacha10Times(
    bucketName: string,
    dir?: string
  ): Promise<string[]> {
    const instance = this.getInstance()
